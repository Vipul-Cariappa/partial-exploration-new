...
//            : 0);
    ...
    Int2ObjectFunction<Int2DoubleFunction> confidenceWidthFunction = state -> (action -> {
      ...
      HashMap<Integer, HashMap<Integer, ArrayList<Integer>>> actionHash = martingaleTransitions.get(state);
      HashMap<Integer, ArrayList<Integer>> nextStateHash = actionHash.get(action);

      double max = Double.NEGATIVE_INFINITY;
      for (var pair: nextStateHash.entrySet()) {
        double[] sample = pair.getValue().stream().mapToDouble(i -> (double) i).toArray();
        double value = BettingMartingale.confidence_width(
                new VectorDouble(sample),
                1000, 0.05, true, 0.5, 0.5);
        if (max < value) {
          max = value;
        }
      }
  ...
  public HashMap<Integer, HashMap<Integer, HashMap<Integer, ArrayList<Integer>>>> martingaleTransitions = new HashMap<>();
  ...
  public void updateMartingaleTransitions(int currentState, int actionIndex, int nextState) {
    ...
    HashMap<Integer, HashMap<Integer, ArrayList<Integer>>> actionHash = martingaleTransitions.get(currentState);

    actionHash.putIfAbsent(actionIndex, new HashMap<>());
    HashMap<Integer, ArrayList<Integer>> nextStateHash = actionHash.get(actionIndex);

    // Get all possible nextStates
    explorer.getActions(currentState).get(actionIndex).distribution().forEach((s, d) -> {
      nextStateHash.putIfAbsent(s, new ArrayList<>());
      ArrayList<Integer> distribution = nextStateHash.get(s);
      if (s == nextState) {
        distribution.add(1);
      } else {
        distribution.add(0);
